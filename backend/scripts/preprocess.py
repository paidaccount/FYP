...
def run_pipeline() -> None:
    ...
    raw_type = featured_df["attacker_type"] if "attacker_type" in featured_df.columns else (featured_df["attackerType"] if "attackerType" in featured_df.columns else 0)
    if isinstance(raw_type, pd.Series):
        featured_df[TARGET_COLUMN] = raw_type.apply(map_raw_attacker_type)
    else:
        featured_df[TARGET_COLUMN] = 0
...
def generate_statistics_and_plots(df: pd.DataFrame) -> None:
    ...
    stats = {
        ...
        "speed_stats": {
            "min": float(df["speed_magnitude"].min()) if "speed_magnitude" in df.columns else 0.0,
            "max": float(df["speed_magnitude"].max()) if "speed_magnitude" in df.columns else 0.0,
            "mean": float(df["speed_magnitude"].mean()) if "speed_magnitude" in df.columns else 0.0
        }
    }
    ...
    try:
        ...
        speed_series = df["speed_magnitude"] if "speed_magnitude" in df.columns else (df["speed"] if "speed" in df.columns else df["speed_x"])
        speed_series.plot(kind="hist", bins=30, color="teal")
        ...
        logger.warning(f"Skipping plot chart image generation: {e}")
